...
Cuando termines el laboratorio, ejecuta el siguiente comando (con tu UO) y sube el Zip correspondiente al campus:
...
Por lo tanto, estás en el paquete `es.uniovi.dlp.parser`, si quieres referenciar la clase `es.uniovi.dlp.ast.expressions.Expression`, por ejemplo, debes incluir su `import` correspondiente.
...
varDefinitions returns [List<VarDefinition> ast = new ArrayList<>()]
    : ids+=ID (',' NL* ids+=ID)* DOUBLE_COLON NL* type NL {
        for(var id : $ids) {
...
Sin embargo, dentro del contexto de una acción, tenemos las variables `start` y  `stop` que son los tokens iniciales y finales que encajan en esa regla, por si queremos usarlos en lugar de definir labels cada vez:
...
Si te fijas en este comando hacemos los siguientes pasos:
